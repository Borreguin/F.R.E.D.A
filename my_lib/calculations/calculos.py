...
from my_lib.PI_connection import pi_connect_fake as osi
...
def demanda_nacional_desde_tag(ini_date=None, fin_date=None, delta=None):
    ...
    # id_tag de demanda nacional
    path_config_file = script_path.replace('my_lib\\calculations', 'hmm\\config.xlsx')
...
def informacion_sankey_generacion_demanda(timestamp=None):
    ...
    df_result = pd.DataFrame(columns=["container", "target", "value", "timestamp"])
    idx = 0
    for detail in details:
        ...
        if df_gen.sum() > 0:
            df_result.at[idx, "container"] = detail
    ...
    if no_convencional_mw > 0:
        df_result.at[idx, "container"] = "No convencional"
...
def informacion_sankey_generacion_demanda_por_provincia(provincia, timestamp=None):
    ...
    # El S.N.I recibe potencia de los generadores locales e inmersa
        ...
        for d, v in zip(details[3:], values):
            idx += 1
            if v > 0:
                df_result.at[idx, "container"] = "Gen. " + provincia
                df_result.at[idx, "value"] = round(v, 1)
...
def flujo_de_lineas(nivel_voltaje, ini_date=None, end_date=None):
    ...
    df_detail = pd.read_excel(os.path.join(script_path,"Cargabilidad_lineas.xlsx"))
    # df_detail["Voltaje"] = df_detail["Voltaje"].astype(float)
    mask = df_detail["Voltaje"] == (nivel_voltaje + " kV")
    ...
    df_detail["ID_circuito"] = [df_detail["CodigoLinea"].loc[x] + "_" + str(df_detail["Circuito"].loc[x])
                                for x in df_detail.index]

    df_detail["CodigoMed"] = [df_detail["CodigoLinea"].loc[x] + "_" + str(df_detail["Circuito"].loc[x]) + "_"
                              + str(df_detail["TipoTag"].loc[x])
                              for x in df_detail.index]

    time_range = pd.date_range(ini_date, end_date, freq="4s")
    df_values = process_simulated_values(df_detail, time_range)

    # trabajando con los puntos .AQ:
    df_quality = process_calidad(df_detail)
    ...
    r = pd.DataFrame(r)
    r.index = r["CodigoMed"]
    r = pd.merge(r, df_detail, on=["CodigoMed", "ID_circuito"])
...
def datos_cargabilidad_lineas(nivel_voltaje, ini_date=None, end_date=None):
    ...
    to_send = dict(data=resp, settings=dict(max=df["Lim_Termico"].max(), min=df["Lim_Termico"].min()))
    return to_send
...
def process_simulated_values(df_work, time_range):
    df_result = pd.DataFrame()
    n = len(time_range)
    columns = ['count', 'mean', 'std', 'min', 'per_25', 'per_50', 'per_75', 'max', "timestamp", "current_value",
               "ID_circuito"]
    x = np.random.randint(0, 30, 1)[0]
    t = time_range[-1] - dt.timedelta(seconds=int(x))
    seed = np.random.uniform(low=-15, high=20, size=(n,))
    for ix in df_work.index:
        values = seed + np.random.uniform(low=-1, high=1, size=(n,))
        values += df_work["Value"].loc[ix]
        df_values = pd.DataFrame(data=values,index=time_range)
        describe = df_values.describe().T
        describe.index = [df_work["CodigoMed"].loc[ix]]
        describe["timestamp"] = t
        snapshot = values[-1]
        try:
            describe["current_value"] = round(snapshot,1)
        except Exception as e:
            describe["current_value"] = None
...
def process_calidad(df_work):
    df_result = pd.DataFrame()
    status = ["Normal", "Normal", "Normal", "Normal", "Normal", "Normal", "Normal", "Normal", "Normal", "TE", "Normal", "Freeze"]
    columns = ["quality"]
    for ix in df_work.index:
        x = np.random.randint(0,10,1)[0]
        describe = pd.DataFrame(index=[df_work["CodigoMed"].loc[ix]], columns=columns)
        describe["quality"] = status[x]
        df_result = df_result.append(describe)
...
def validar_flujo(df_to):
    ...
    try:
        ...
        df_to["dif"] = np.random.randint(-10, 10, 1)[0] / 10
    except Exception as e:
        print(e)
        #df_to["dif"] = 0
        df_to["dif"] = np.random.randint(-10,10,1)[0]/10

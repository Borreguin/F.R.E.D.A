...
def day_cluster_matrix(hmm_model, df_y, model_id):
    ...
    for n in range(n_comp):
        ...
        df_aux.index = [x.day_name() for x in df_aux.index]
    ...
    dict_cl_week["free_to_free"] = hl.get_free_to_free()
    dict_cl_week["free_to_work"] = hl.get_free_to_work()
    dict_cl_week["work_to_free"] = hl.get_work_to_free()
...
def get_expected_profiles_from(df_mean, df_with, n_expected_clusters):
    ...
    df_error.iloc[-1] = df_error.iloc[-1] * 100
    try:
...
def obtain_expected_area(model_path, data_path, df_tag, n_columns=None):
    ...
    """
    df_int = df_tag.copy()
    d_week = df_tag.index[0].day_name()
    ...
    for family in profile_families:
        try:
            ...
            for n_profiles in range(0, n_profiles_to_see + 1):
                ...
                df_expected_area, alpha_min_max = adjust_expect_band(df_int.ix[:, 0], df_profile, df_std, n_columns)
                ...
                if error < min_error:
                    ...
                    result["expected_profiles"] = [int(x) for x in expected_profiles]
...
def adjust_expect_band(df_int, df_profile, df_std, dt_index=None):
    """
    ...
    :param dt_index:    Index for the answer
...
def estimate_error(df_real, df_predicted):
    ...
    df_error = df_error / (df_real+0.01)
...
def graphic_pronostico_demanda(hmm_modelPath, file_dataPath, df_tag, despacho, datetime_ini, datetime_fin, style):
    ...
    """elif despacho == "None":
            df_despacho = pd.DataFrame(columns=["Valor esperado"])
            ix = pd.date_range(df_expected.index[0], df_expected.index[-1], freq="60T")
            df_despacho["Valor esperado"] = df_expected.loc[ix]

            # df_despacho = pd.DataFrame()
        """
    # TODO: Delete dataframe about despacho
    df_despacho = pd.DataFrame()
...
def traces_expected_area_and_real_time(df_expected_area):
    ...
    names = {'min': 'Min valor esperado', 'max': 'Max valor esperado', 'real time': 'Valor actual',
